...
from inkex.tween import AttributeInterpolator, StyleInterpolator, EqualSubsegmentsInterpolator, FirstNodesInterpolator
...
class Interp(inkex.EffectExtension):
    ...
    def add_arguments(self, pars):
        pars.add_argument("-e", "--exponent", type=float, default=1.0,\
            help="values other than zero give non linear interpolation")
        pars.add_argument("-s", "--steps", type=int, default=5,\
            help="number of interpolation steps")
        pars.add_argument("-m", "--method", type=str, default="equalSubsegments",\
    ...
    def effect(self):
        ...
        for (elem1, elem2) in objectpairs:
            ...
            path_interpolator = AttributeInterpolator.create_from_attribute(elem1, elem2, "d",
                                                                            method=method)
    ...
    def get_steps(self):
        ...
        steps = [((i+1) / (self.options.steps + 1.0))** exponent for i in range(self.options.steps)]
        if self.options.dup:
            steps = [0] + steps + [1]
        return steps
