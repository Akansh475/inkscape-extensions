...
class hpglEncoder(object):
        ...
        if self.toolOffset > 0.0 and self.options.precut:
            if self.options.center:
                ...
                if self.offsetY >= 0.0:
                    ...
                    precutY = self.offsetY - self.toolOffset
                self.processOffset('PU', precutX, precutY, self.options.pen, self.options.speed, self.options.force)
                self.processOffset('PD', precutX, precutY + self.toolOffset * 8, self.options.pen, self.options.speed, self.options.force)
            else:
                self.processOffset('PU', 0, 0, self.options.pen, self.options.speed, self.options.force)
                self.processOffset('PD', 0, self.toolOffset * 8, self.options.pen, self.options.speed, self.options.force)
        # start conversion
        self.process_group(self.doc, transform)
        # shift an empty node in in order to process last node in cache
        if self.toolOffset > 0.0 and not self.dryRun:
            self.processOffset('PU', 0, 0, 0, 0, 0)
        return self.hpgl
            ...
            for singlePath in path:
                ...
                for singlePathPoint in singlePath:
                    ...
                    if int(round(posX)) != int(round(oldPosX)) or int(round(posY)) != int(round(oldPosY)):
                        self.processOffset(cmd, posX, posY, pen, speed, force)
                        cmd = 'PD'
                        oldPosX = posX
                        oldPosY = posY
                # perform overcut
                        ...
                        for singlePathPoint in singlePath:
                            ...
                            if int(round(posX)) != int(round(oldPosX)) or int(round(posY)) != int(round(oldPosY)):
                                ...
                                if overcutLength >= self.overcut:
                                    ...
                                    self.processOffset(cmd, newLength[0], newLength[1], pen, speed, force)
                                    break
                                else:
                                    self.processOffset(cmd, posX, posY, pen, speed, force)
                                oldPosX = posX
    ...
    def processOffset(self, cmd, posX, posY, pen, speed, force):
        # calculate offset correction (or don't)
        if self.toolOffset == 0.0 or self.dryRun:
            self.storePoint(cmd, posX, posY, pen, speed, force)
        else:
            # insert data into cache
            self.vData.pop(0)
            self.vData.insert(3, [cmd, posX, posY, pen, speed, force])
            # decide if enough data is available
            if self.vData[2][1] != 'False':
                if self.vData[1][1] == 'False':
                    self.storePoint(*self.vData[2][:6])
                else:
                    ...
                    if self.vData[2][0] == 'PD': # If the 3rd entry in the cache is a pen down command make the line longer by the tool offset
                        ...
                        self.storePoint('PD', pointThree[0], pointThree[1], self.vData[2][3], self.vData[2][4], self.vData[2][5])
                        ...
                        self.storePoint('PU', pointThree[0], pointThree[1], self.vData[2][3], self.vData[2][4], self.vData[2][5])
                    else:
                    ...
                    if self.vData[3][0] == 'PD':
                        ...
                        if angleVector >= 0:
                            ...
                            while angle < angleStart + angleVector:
                                self.storePoint('PD', self.vData[2][1] + math.cos(angle) * self.toolOffset, self.vData[2][2] + math.sin(angle) * self.toolOffset, self.vData[2][3], self.vData[2][4], self.vData[2][5])
                            ...
                            while angle > angleStart + angleVector:
                                self.storePoint('PD', self.vData[2][1] + math.cos(angle) * self.toolOffset, self.vData[2][2] + math.sin(angle) * self.toolOffset, self.vData[2][3], self.vData[2][4], self.vData[2][5])
                                angle -= self.toolOffsetFlat
                        self.storePoint('PD', pointFour[0], pointFour[1], self.vData[3][3], self.vData[3][4], self.vData[3][5])

    def storePoint(self, command, x, y, pen, speed, force):
        x = int(round(x))
        y = int(round(y))
