...
class BaseStyleValue():
    ...
    def factory(cls, declaration: Optional[str] = None, attr_name: Optional[str] = None, \
        ...
        """
        if (declaration is not None and ':' in declaration):
            attr_name, value, important = BaseStyleValue.parse_declaration(
                declaration)
...
class URLNoneValue(BaseStyleValue):
    ...
    def element_has_root(element) -> bool:
        ...
        return not (element.getparent() is None and \
            not isinstance(element, SvgDocumentElement))
        ...
        for curkey in dct:
            ...
            if curkey not in current_keys:
                ...
                if importance != style.get_importance(curkey):
                    ...
                    perform = current_keys.index(
                        curkey) < current_keys.index(self.attr_name)
...
class FontValue(ShorthandValue):
    ...
    def get_shorthand_changes(self):
        keys = ["font-style", "font-variant", "font-weight",
                "font-stretch", "font-size", "line-height", "font-family"]
        options = {key: all_properties[key][4] for key in keys if isinstance(
            all_properties[key][4], list)}
...
all_properties: Dict[str, Tuple[Type[BaseStyleValue], str, bool, bool, Union[List[str], None]]] = {
    "alignment-baseline": (EnumValue, "baseline", True, False, ["baseline", "text-bottom", "alphabetic", "ideographic", "middle", "central", "mathematical", "text-top"]),
    ...
    "color-interpolation": (EnumValue, "sRGB", True, True, ["sRGB", "auto", "linearRGB"]),
    "color-interpolation-filters": (EnumValue, "linearRGB", True, True, ["auto", "sRGB", "linearRGB"]),
    "color-rendering": (EnumValue, "auto", True, True, ["auto", "optimizeSpeed", "optimizeQuality"]),
    "cursor": (BaseStyleValue, "auto", True, True, None),
    "direction": (EnumValue, "ltr", True, True, ["ltr", "rtl"]),
    "display": (EnumValue, "inline", True, False, ["inline", "block", "list-item", "inline-block", "table", "inline-table", "table-row-group",
                                                   "table-header-group", "table-footer-group", "table-row", "table-column-group", "table-column",
                                                   "table-cell", "table-caption", "none"]),  # every value except none is rendered normally
    "dominant-baseline": (EnumValue, "auto", True, True, ["auto", "text-bottom", "alphabetic", "ideographic", "middle", "central", "mathematical", "hanging", "text-top"]),
    "fill": (PaintValue, "black", True, True, None), # the normal fill, not the <animation> one
    ...
    "overflow": (EnumValue, "visible", True, False, ["visible", "hidden", "scroll", "auto"]),
    "paint-order": (BaseStyleValue, "normal", True, False, None),
    "pointer-events": (EnumValue, "visiblePainted", True, True, ["bounding-box", "visiblePainted",
    ...
    "text-align": (BaseStyleValue, "start", True, True, None), # only HTML property, but used by some unit tests
    ...
    "text-rendering": (EnumValue, "auto", True, True, ["auto", "optimizeSpeed", "optimizeLegibility", "geometricPrecision"]),
    "unicode-bidi": (EnumValue, "normal", True, False, ["normal", "embed", "isolate", "bidi-override", "isolate-override", "plaintext"]),
